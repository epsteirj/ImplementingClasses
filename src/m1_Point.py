...
class Point(object):
    ...
    def move_to(self,x,y):
        self.movetotal += (((self.y - y) ** 2) + ((self.x - x) ** 2)) ** .5
        self.x=x
        self.y=y
        self.moves+=1
    def move_by(self,dx,dy):
        self.movetotal += (((self.y - self.y+dy) ** 2) + ((self.x - self.x+dx) ** 2)) ** .5
        self.x=self.x+dx
        self.y=self.y+dy
        self.moves+=1
    ...
    def get_distance_from(self,p2):
        return (((p2.y-self.y)**2)+((p2.x-self.x)**2))**.5
    def get_distance_from_start(self):
        return (((self.y1-self.y)**2)+((self.x1-self.x)**2))**.5
    def get_distance_traveled(self):
        return self.movetotal
    def closer_to(self,p2,p3):
        if ((((p2.y - self.y) ** 2) + ((p2.x - self.x) ** 2)) ** .5 <= ((((p3.y-self.y)**2)+((p3.x-self.x)**2))**.5)):
            return p2
        else:
            return p3
    def halfway_to(self,p2):
        self.x=((self.x+p2.x)/2)
        self.y=((self.y+p2.y)/2)
        return 'Point('+str(self.x)+','+str(self.y)+')'
